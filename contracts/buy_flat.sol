...
contract Buy is AccessControl {
     ...
     *  @param _hypeToken Hype token address 
     *  @param _usdToken BUSD token address   
     *  @param _directRoundRate BUSD/HYPE exchange rate  
     *  @param _directRoundLimit limitation of tokensale round
     *  @param _orderLimit min amount of order(in BUSD)
     */
    constructor( 
        address _hypeToken,
        address _usdToken,
        ...
        usdToken = _usdToken;
        directRoundRate = _directRoundRate;
        directRoundLimit = _directRoundLimit * 10 ** ERC20(_usdToken).decimals();
        orderLimit = _orderLimit * 10 ** ERC20(_usdToken).decimals();
    }

    /** @dev initialize of new agent
     *  @param _promoCodeDigest sha3 digest of agent's promo code   
     *  @param _agent address of agent wallet   
     */
    function initAgent(
    ...
    /** @dev swap BUSD for HYPE tokens from this contract
     *  @param _value BUSD amount
     *  @param _promoCodeDigest sha3 of promo code
     */
    function swap(
        uint256 _value,
        bytes32 _promoCodeDigest
            ...
            // transfer BUSD from buyer to contract
            ERC20(usdToken).safeTransferFrom(msg.sender, address(this), _value);
            // transfer HYPE from contract to buyer
            ...
            // transfer BUSD from contract to agent 
     ...
     *  @param _to address, who gets tokens
     *  @param _to address, who gets tokens
     */
    function claim(address _asset, address _to, uint256 _amount) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        ERC20(_asset).safeTransfer(_to, _amount  * 10 ** ERC20(_asset).decimals());
    }
